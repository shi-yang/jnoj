...
void UVAJudger::login() {
  ...
  // check login status
  string html = loadAllFromFile(tmpfilename);
  if (html.find("<td>No account yet? ") != string::npos ||
 ...
 */
int UVAJudger::submit(Bott * bott) {
 ...
 */
Bott * UVAJudger::getStatus(Bott * bott) {
  ...
  while (true) {
    ...
    // get first row
    if (html.find("<b>One or more following ERROR(s) occurred.") !=
            string::npos ||
...
/**
 * Get compile error info from UVA
 * @param bott      Result bott file
 * @return Compile error info
 */
string UVAJudger::getCEinfo(Bott * bott) {
  prepareCurl();
  curl_easy_setopt(
      curl, CURLOPT_URL,
 ...
 */
string UVAJudger::convertResult(string result) {
