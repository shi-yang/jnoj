...
<ol>
    ...
    <li>不建议要求采用“输入 t, 表示 t 组数据”之类的题面，建议一个测试点一组样例，即类似于 CF 或者 World Final 那样题面即可。
        为保证能够完整测试程序，建议测试点在 30 左右，如无法满足测试，则应控制在 150 以内。需<b>根据题目来合理选择测试点的数量。</b>
    </li>
    <li>
...
<ol>
    ...
    <li><b>必须写明所提到的所有变量的范围</b>。范围描述应当使用 ≤，而不是 ＜。如果提到一个字符串，必须写明哪些字符可以出现在这个字符串内。</li>
    <li>题目描述数组下标最好从 1 开始。</li>
    <li>如果是中文题面的题目，请正确使用标点符号，不要混用中英文标点(中文使用全角，英文使用半角)。</li>
    <li>OJ 支持 Markdown 语法与 Katex 语法。题目出现的<b>所有变量名应使用 Katex 语法来包含，数据范围的描述也用 Katex 来写。</b>，参考题目规范示例</li>
