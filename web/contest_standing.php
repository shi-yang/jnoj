...
if ($admin == 1) $pagetitle .= " for Admin";
$realnow = time();
$chaing = false;
$ccpassed = false;
$cidtype = array();
...
if (contest_exist($cid)) {
    ...
    if (contest_passed($cid)) {
        ?>
        <div class="slidediv" style="width:90%;margin:5px auto">
            <span class="passtime"><?= get_time($nowtime - $sttimeu, true) ?></span>
            <div class="timeslider noUiSlider" style="width:100%" name="<?= $nowtime - $sttimeu ?>"></div>
    ...
    <div class="tcenter currentstat">
        <label class="radio-inline"><input type="radio" id="stat_dis_nick">Display Nickname</label>&nbsp;&nbsp;&nbsp;&nbsp;
        <label class="radio-inline"><input type="radio" id="stat_dis_user" checked>Display Username</label><br/>
        <b>
            ...
            else if ($realnow > $locktu) {
                echo "Board Locked";
                if ($admin == 1) echo " (Not for admin)";
...
//                print_r($a);
        ...
        $wdt = (sizeof($map) * 100 + 200) . "px";

        echo "<table style='width:$wdt;margin:auto;' class='table table-striped table-hover cstanding basetable'>";
        echo "\n" . '<thead><tr>';
        echo "<th class='trank anim:position'> # </th>";
        ...
        if (contest_get_val($cid, "has_cha")) {
            ?>
            <div id="cchainfo" class="modal fade">
                        ...
                        <form id="cchaform" action="#" method="post">
                            <div class="modal-body">
                                ...
                                if (contest_challenging($cid) || contest_intermission($cid)) {
                                    ?>
                                    <b>Data Type: </b><input type='radio' name='chadata_type' value='0'
                                                             checked="checked"
                                                             style="width:20px"> Raw Data &nbsp;&nbsp;&nbsp;&nbsp;
                                    <input type='radio' name='chadata_type' value='1' style="width:20px"> Souce Code
                                    <br/>
                                    ...
                                    <b>Data Detail: </b><br/>
                                    <textarea rows="16" name="chadata_detail" style="width:650px"
                                              onKeyUp="if(this.value.length > 32768) this.value=this.value.substr(0,32768)"></textarea>
                                    <br/>
                            ...
                            <div id="cchadetail" style="display:none;margin-bottom:10px">
                                ...
                                <div class="well" id="cchadetailcontent"></div>
                            </div>
                            <h4>Challenge History</h4>
                            <div class="well" id="cchahistory"></div>
                            <?php
                            if (contest_challenging($cid) || contest_intermission($cid)) {
                                ?>
                                <div class="modal-footer">
                                    <span id="msgbox" style="display:none;"></span>
                                    <input type='submit' value='Challenge!' class="btn btn-primary"/>
                                </div>
                                <?php
                            }
                            ?>
