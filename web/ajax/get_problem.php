...
if ($current_user->is_root()) {
    ...
    if ($db->num_rows > 0) {
        $res["code"] = 0;
        list($res["pid"], $res["title"], $res["desc"], $res["inp"], $res["oup"], $res["sinp"], $res["sout"], $res["noc"], $res["spj"], $res["tl"], $res["ctl"], $res["ml"], $res["hint"], $res["source"], $res["p_hide"], $res["p_ignore_noc"], $res["author"]) = $db->get_row($result, ARRAY_N);
